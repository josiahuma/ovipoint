// app/[churchSlug]/admin/events/page.tsx
...
export default async function ChurchEventsAdminPage({ params }: PageProps) {
  ...
  // booking counts per event (fetch bookings and count client-side to avoid unsupported 'group' in SDK types)
  const { data: bookings } = allEvents.length
    ? await supabase
        .from('bookings')
  ...
  const bookingCountMap: Record<number, number> = {};
  (bookings || []).forEach((row: any) => {
    const id = Number(row.pickup_event_id);
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-4 space-y-4">
          ...
          {eventsError && (
            <p className="text-red-600 text-sm">
              Failed to load events for this organisation.
            </p>
          )}
                ...
                {upcomingEvents.map((event: any) => {
                  ...
                  const bookingsOpen =
                    ...
                    event.bookings_open === undefined
                      ? true
                      : !!event.bookings_open;
                  const totalBookings = bookingCountMap[event.id] ?? 0;
